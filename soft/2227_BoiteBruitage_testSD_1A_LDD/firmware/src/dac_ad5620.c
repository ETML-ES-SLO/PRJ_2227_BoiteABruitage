...
void Dac_Init(void)
{
    ...
    /* Baud rate selection */
    PLIB_SPI_BaudRateSet( SPI_ID_2 , SYS_CLK_PeripheralFrequencyGet(CLK_BUS_PERIPHERAL_1), 1000000 );
...
void Dac_Write( uint16_t ech)
{
    ...
    ech = ech <<2 ;
    //mise à 0 les valeurs de choix du mode
    ech &= 0x3FFF;
            
    //envoi des trames sur le DAC
    CS_DACOff();
    spi_write2(ech >> 8);
    spi_write2(ech);
    CS_DACOn();
